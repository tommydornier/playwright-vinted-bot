...
async function handleAppleLogin(page, context, credentials) {
  ...
  // Cliquer sur le bouton "Continuer" pour valider l’e-mail
  ...
  await applePage.click('button[aria-label="Continuer"]', { force: true });
  await applePage.waitForTimeout(1000);
  ...
  // Au lieu de se baser sur l'aria-label, on recherche le bouton contenant l'icône de validation
  console.log("Recherche et clic sur le bouton de validation (icône 'icon_sign_in')...");
  await applePage.waitForSelector('button:has(i.shared-icon.icon_sign_in)', { timeout: 60000 });
  await applePage.click('button:has(i.shared-icon.icon_sign_in)', { force: true });
